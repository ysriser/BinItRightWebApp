...
@CrossOrigin(origins = "*")
public final class RewardShopController {
    private final RewardShopService rewardShopService;
    private final JwtUtil jwtUtil;

    public RewardShopController(final RewardShopService rewardShopService, final JwtUtil jwtUtil) {
    ...
    public ResponseEntity<RedeemResponse> redeem(@PathVariable final Long accessoriesId,
                                                 final Authentication authentication) {
        final String userIdStr = (String) authentication.getPrincipal();
        final Long userId = Long.valueOf(userIdStr);
        final RedeemResponse response = rewardShopService.redeem(userId, accessoriesId);
